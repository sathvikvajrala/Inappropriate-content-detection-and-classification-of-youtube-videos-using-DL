...
def graph():
    df = pd.DataFrame([['EfficientNet-BiLSTM','Precision',precision[0]],['EfficientNet-BiLSTM','Recall',recall[0]],['EfficientNet-BiLSTM','F1 Score',fscore[0]],['EfficientNet-BiLSTM','Accuracy',accuracy[0]],
                       ...
                       ['EfficientNet-RFC','Precision',precision[2]],['EfficientNet-RFC','Recall',recall[2]],['EfficientNet-RFC','F1-score',fscore[2]],['EfficientNet-RFC','Accuracy',accuracy[2]]],
                       columns=['Parameters','Algorithms','Value'])
    
    # Using pivot_table for aggregation
    df.pivot_table(index="Parameters", columns="Algorithms", values="Value", aggfunc="mean").plot(kind='bar')
...
rfButton = Button(main, text="Generate & Load EfficientNet-RF Model", command=rf)
